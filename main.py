...
def train(**kwargs):
    ...
    while True:
      ...
      try:
        ...
        print('Fail ', time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))
        time.sleep(60)
...
def train_x(**kwargs):
    ...
    optimizer_G = t.optim.Adam(itertools.chain(G_A2B.parameters(), G_B2A.parameters()), 
                               lr=opt.lr, betas=(0.5, 0.999))
    optimizer_D = t.optim.Adam(D.parameters(), lr=opt.lr, betas=(0.5, 0.999))
    
    vis = Visualizer(opt.env)
    vis.add_names('loss_G', 'loss_D')
    ...
    for epoch in range(opt.n_epoch):
        ...
        for i, (real_A, real_B, img_names_A, img_names_B) in enumerate(train_dataloader):
            ...
            label_B = t.ones(real_A.size()[0], 1).cuda()
            label_A = t.zeros(real_A.size()[0], 1).cuda()
            ...
            loss_D_A = criterion_real(D(gener_A), label_B) + criterion_real(D(real_A), label_A)
            loss_D_A = loss_D_A*0.5
            
            loss_D_B = criterion_real(D(gener_B), label_A) + criterion_real(D(real_B), label_B)
            loss_D_B = loss_D_B*0.5
            ...
            avgloss_G += loss_G.cpu().item()
            avgloss_D += loss_D.cpu().item()
        
        vis.plot('loss_G', epoch, avgloss_G/i)
        vis.plot('loss_D', epoch, avgloss_D/i)
        ...
        vis.log('epoch:{}, loss_G:{}, loss_D:{}'.format(epoch, avgloss_G/i, avgloss_D/i))
